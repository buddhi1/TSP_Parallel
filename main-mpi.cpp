...
int main(int argc, char *argv[]) 
{
	...
	if (id == 0)
	{
		...
		for (int pid = 1; pid < procCount; ++pid)
		{
    		...
    		MPI_Send(graph, (vr-1)*(vr-1), MPI_INT,pid,1, MPI_COMM_WORLD);			
			blockSize = numPerm / procCount;			
		}
	    ...
	    // MPI_Reduce(&tmpMin, &minPath, 1, MPI_INT, MPI_MAX, 0, MPI_COMM_WORLD);


		// for (int pid = 1; pid < procCount; ++pid)
		// {
  //   		MPI_Recv(&tmpMin, 1, MPI_INT, pid, 2, MPI_COMM_WORLD, &status);
  //   		if (tmpMin < minPath)
  //   		{
  //   			minPath = tmpMin;
  //   		}
	    ...
	    MPI_Recv(graph, (vr-1)*(vr-1), MPI_INT, 0, 1, MPI_COMM_WORLD, &status);	
	    tmpMin = TSP_parallel(graph, origin, blockSize);
	    

	    // MPI_Send(&minPath, 1, MPI_INT, 0, 2, MPI_COMM_WORLD);
	}
